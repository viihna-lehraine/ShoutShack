...
const compressAndExportLogs = async (sourceDir, exportDir, logFileName) => {
    ...
    try {
        ...
        logger.error(`Unable to compress ${logFileNme}: ${err.message}`);
...
const runCommandAndLog = (command, logFilePath) => {
    return new Promise((resolve, reject) => {
        ...
        const process = exec(command, { maxBuffer: 1024 * 500 }, (error, stdout, stderr) => {
            if (error) {
...
const exportLogs = async() => {
    ...
    const exportDir = path.join(__dirname, '../../../.logExports');
    ...
    try {
        ...
        const serverLogFiles = fs.readdirSync(serverLogDir).filter(file => file.endsWith('.log'));
        ...
        const npmLogFiles = fs.readdirSync(npmLogDir).filter(file => file.endsWith('.logs'));
