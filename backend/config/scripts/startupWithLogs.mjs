...
const logDir = path.resolve('data/logs/server/startup');
const logNumber = fs.readdirSync(logDir).length + 1;
const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
const logFilePath = path.join(logDir, `startupLog-${logNumber}.log`);
