...
let backupsDir = path.join(__dirname, '../../data/dist-backups');
...
const getBundleNumber = () => {
	...
	let bundleNumbers = files
		.map((file) => {
			const match = file.match(/^distBackupBundle(\d+)\.tar\.gz$/);
			return match ? parseInt(match[1], 10) : null;
		})
		.filter((num) => num !== null);
...
const createSrcBackup = () => {
	...
	let backupFileName = `distBackup${backupNumber}.tar.gz`;
	...
	tar.c(
		...
		.then(() => {
			...
			fs.unlinkSync(path.join(distDir, timestampFileName));
		})
...
const bundleTarballs = () => {
	let tarballs = fs
		...
		.filter(
			(file) =>
				file.endsWith('.tar.gz') && !file.startsWith('distBackupBundle')
		);
